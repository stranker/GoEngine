...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    ...
    // combine results
    vec3 ambient = material.ambient * light.color * ((material.hasTexture) ? vec3(texture(material.texture_diffuse, TexCoords)) : vec3(1.0));
    vec3 diffuse = material.diffuse * diff * ((material.hasTexture) ? vec3(texture(material.texture_diffuse, TexCoords)) : vec3(1.0));
    vec3 specular = material.specular * light.specular * spec * ((material.hasTexture) ? vec3(texture(material.texture_diffuse, TexCoords)) : vec3(1.0));
